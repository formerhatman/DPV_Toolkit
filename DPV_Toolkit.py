...
class disto(QtGui.QWidget):
    '''
    class for the distributions plot in the distributions tab
    '''
...
class grapho(pg.PlotWidget):
    '''
    class for the scatterplots, used in modeling tab and noise removal.
    '''
    ...
    def graphene(self,*args,**kwargs):
        ...
        if 'data' in self.ploot.__dict__.keys():
            pass
    ...
    def grouping(self,kwds):
        ...
        self.ploot.getViewBox().setRange(xRange = (means[self.X].min() - (2.5*stds[self.X].max()),means[self.X].max() + (2.5*stds[self.X].max())), yRange = (means[self.Y].min() - (0.5*stds[self.Y].max()),means[self.Y].max() + (stds[self.Y].min())*0.5))
        self.legendo.setPos(means[self.X].min() - (2.5*stds[self.X].max()),means[self.Y].max() + (stds[self.Y].min())*0.5)     
...
'''
Here begins the layout of the app:
'''
...
def get_filename(filespot,graphspot,rolling=False):
    '''
    grabs the filename and loads the data into the scatterplot.
    Change the column names at your own risk - she crashes when she's angry. 
    '''
    ...
    if rolling==False:
        ...
        #initial graphing for modeling tab uses rolling means to speed up graphing time, since calculation of rolling mean is faster than drawing points. 
